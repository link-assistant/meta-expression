...
| R44 | Translate through formalized Wikidata entities.               | MVP   | Translation first runs `/formalize`, then swaps resolved Q/P phrases for target-language Wikidata labels and sitelinks.  | Add grammatical reordering and predicate/fact translation rules.                                                |
| R45 | Emit a formalization CST rich enough to regenerate Markdown.  | MVP   | `formalizeTextWith()` returns a CST with phrase ids, token spans, entity ids, URLs, candidates, and contexts.            | Move the CST to a parser-backed Links Notation representation when the Links Notation dependency is selected.   |
...
| R47 | Keep translation tests fact-based and reproducible.           | MVP   | Issue #16 tests mock Wikidata search/entity responses and assert CST, Markdown, CLI, HTTP, and web wiring.               | Add snapshot-backed integration tests for larger multilingual examples.                                         |
