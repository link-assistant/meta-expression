...
**Status:** Analysis and first JavaScript prototype implemented
**Created:** 2026-04-26
...
formal reasoning, evidence retrieval, probability estimation, and links network
...
complete fact checking. It should instead implement a narrow links-network-first
pipeline:
...
input text
  ...
  -> formalization level
  -> dependency/evidence links network
...
| [`pr-2-conversation-comments.json`](./data/pr-2-conversation-comments.json) | PR discussion comments, including prototype feedback |
| [`pr-2-review-comments.json`](./data/pr-2-review-comments.json)             | PR inline review comments                            |
| [`pr-2-reviews.json`](./data/pr-2-reviews.json)                             | PR review summaries                                  |
...
The current PR now contains a first deterministic JavaScript prototype while
the Rust/WASM/React/Doublets implementation remains planned in the roadmap.
...
| [`link-foundation/links-notation`](https://github.com/link-foundation/links-notation)           | Internal and external representation       | Multi-language parser/serializer for Links Notation across JavaScript, Rust, C#, Python, Go, and Java.                                      |
...
| [`linksplatform/doublets-rs`](https://github.com/linksplatform/doublets-rs)                     | Persistent links-network storage           | Rust link-store implementation with file-mapped storage, query patterns, and FFI support.                                                   |
...
| [`link-foundation/meta-theory`](https://github.com/link-foundation/meta-theory)                 | Terminology and links-theory reference     | Confirms the product should model a network of links, where visual projections must not become the core terminology.                        |
...
| Links-network UI    | [React Flow](https://reactflow.dev/learn), [Cytoscape.js](https://js.cytoscape.org/)                                                                                     | Candidate renderer libraries if adapted to links-network terminology and data.                                           |
...
2. Core representation: every statement, interpretation, dependency,
   evidence item, and computation result is a versioned link.
...
4. Formalization: move statements through explicit levels from raw text to
   structured links network, partial formal expression, and fully computable
   expression.
...
8. Scope guardrails: no full NLU, perfect probability, complete ambiguity
   coverage, or large-scale links-network optimization in the first prototype.
...
| ---------- | ---------------- | -------------------------------------------------------------------------------------------- |
| `core`     | Rust             | Statement links network, formalization levels, probability model, Links Notation conversion. |
| `wasm`     | Rust/WASM        | Browser bindings for `core`.                                                                 |
| `web`      | React/TypeScript | GitHub Pages playground, interpretation selector, links-network visualization.               |
...
| ----------------------- | --------------------------------------------------------------------------------------- |
| `StatementLink`         | `id`, `text`, `formalizationLevel`, `selectedInterpretationId`, `version`, `provenance` |
| `Interpretation`        | `id`, `statementId`, `paraphrase`, `examples`, `confidence`, `source`                   |
| `StatementRelationLink` | `id`, `source`, `target`, `kind`, `weight`, `provenance`                                |
...
1. Normalize input text and create a level-1 raw statement link.
...
3. Cluster/merge near-equivalent interpretations and present paraphrases to the
   user.
4. Convert the selected interpretation into a level-2 links network and, when possible,
...
- Define Links Notation and JSON fixtures for statements, interpretations,
  relation links, evidence, and results.
...
- Build React UI with input, interpretation selector, result panel, and links network
  visualization.
...
| Ambiguity explosion                             | Links network and UI become unreadable | Limit top-k interpretations, cluster equivalents, and prune by relevance/weight.     |
...
5. Should evidence be persisted locally in Doublets during MVP, or can MVP use
   in-memory links-network storage with export/import first?
...
| `Earth orbits the Sun` | Non-computable real-world claim, Wikidata-backed evidence links network, confidence estimate with provenance. |
...
- Library API in `src/index.js` with `createStatementDraft`,
  `analyzeStatement`, confidence calculation, and Links Notation serialization.
- CLI in `src/cli.js` with JSON and Links Notation output.
- HTTP microservice in `src/server.js` with `/health` and `/analyze`.
- Static web playground in `web/`.
- Tests for explicit interpretation selection, exact arithmetic confidence,
  Wikidata-backed fixture evidence, and links-network serialization.
...
should be a constrained playground with explicit user choice, transparent links network
state, and narrow evidence retrieval. The most important implementation
decision is to make links/provenance/versioning the stable core now, while
keeping LLMs, Wikipedia extraction, richer probability propagation, and
large-scale links-network optimization as replaceable later layers.
