...
| Mirror in Rust core.                                                                                            | Implemented | `rust/core/src/lib.rs` exposes `extract_first_statement`, `tokenize_for_match`, `token_coverage`, `normalize_statement_key`, the `TranslationQualityStatus` enum, and the `meta_expression_translation_quality_status_code` C ABI surface. |
| Tests at all levels: unit, integration, CLI, e2e.                                                               | Implemented | `tests/issue-43.test.js` covers unit helpers, status routing, CLI happy/sad paths, Wikipedia integration, and the skip-list cross-check. Rust unit tests in `rust/core/src/lib.rs` cover the mirrored helpers.                             |
