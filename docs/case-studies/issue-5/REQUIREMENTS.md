...
| 5   | Use Wikipedia/Wikidata for real-world checking.                                       | Next    | Start with Wikidata structured data; use Wikipedia text later.                 | Add worker-based entity/property search and scoped statement retrieval after the deterministic fixtures stabilize.            |
| 6   | If a person is stated dead, use that as refutation/support as appropriate.            | Next    | Model death-date evidence through P570.                                        | Treat present P570 as refuting an alive claim; missing P570 as bounded support only, never proof.                             |
...
| 19  | Traverse Wikipedia in a worker thread.                                                | Next    | Use Web Workers for browser-side retrieval/traversal.                          | Worker fetches selected interpretation evidence and posts links-network updates to UI.                                        |
...
| 21  | Work internally in binary links for Doublets.                                         | Later   | Keep JS links-network schema compatible with future Rust/Doublets mapping.     | Define binary link/value encoding in Rust and test parity with JS fixtures.                                                   |
...
| 23  | Infer support/refutation candidates for a statement.                                  | Next    | Derive candidate properties/entities from selected interpretation.             | Add controlled relation templates first, then optional LLM suggestions with no truth authority.                               |
