 ...
 *   - tags every generated fallback as `source: "rule-derived"` and attaches
 *     the real `attestedForms` Wiktionary lists to the side report, so a human
 *     reviewer can see whether the surface form was derived from an attested
 *     lemma or from the offline rule fallback.
 *
 * Usage:
...
async function recordConcept(concept, args, userAgent, stats, report) {
  ...
  if (proven) {
    ...
    concept.derivation = ruleDerivedDerivation(attestedForms);
    stats.ruleDerived += 1;
    process.stdout.write(
      `  · ${concept.id} rule-derived (${
  ...
  report.push({
    id: concept.id,
    source,
    status: proven ? 'wiktionary' : 'rule-derived',
...
function ruleDerivedDerivation(attestedForms) {
  const source =
    Object.keys(attestedForms).length > 0
      ? 'wiktionary-attested-forms'
      : 'unified-term-data-source';
  return {
    strategy:
      source === 'wiktionary-attested-forms'
        ? 'lexical-inflection-from-attested-lemma'
        : 'offline-rule-derived-fallback',
    source,
  };
}
...
async function main() {
  ...
  const stats = { checked: 0, verified: 0, ruleDerived: 0, skipped: 0 };
  const report = [];
  ...
  process.stdout.write(
    `\nChecked ${stats.checked}, verified ${stats.verified}, ` +
      `rule-derived ${stats.ruleDerived}, skipped ${stats.skipped}.\n`
  );
  if (args.dryRun) {
  ...
  const reportPayload = {
    description:
      ...
      'translation template term. Concepts with status "rule-derived" list the real ' +
      'attested lemmas when available; their surface form is generated by an offline ' +
      'derivation rule rather than stored as an ad hoc override.',
    capturedAt: new Date().toISOString(),
    proven: stats.verified,
    ruleDerived: stats.ruleDerived,
    concepts: report,
  };
